...
const AdminGrades = () => {
    ...
    const loadGrades = async () => {
      ...
      try {
        ...
        const [studentList, gradeRes] = await Promise.all([
          ...
          getGrades({ className, subject: selectedSubject })
        ]);
  ...
  const handleSaveAll = async () => {
    ...
    try {
      ...
      const selectedClassRecord = classes.find(c => (c.fullClassName || c.name) === selectedClass);
      const className = selectedClassRecord?.fullClassName || selectedClass;
      await postGrades({ className, subject: selectedSubject, term: "Third Term", session: "2025/2026", records });
  ...
  return (
    <div className="space-y-8">
      <div className="border-b border-slate-800/80 pb-6">
        ...
        <h1 className="text-3xl font-serif font-bold text-white mt-1">Gradebook & Assessment Entry</h1>
      </div>

      <div className="flex gap-4">
        <select value={selectedClass} onChange={(e) => setSelectedClass(e.target.value)} className="bg-slate-900/50 border-slate-800 rounded-lg px-4 py-2 text-sm text-white">
          {classes.map(c => <option key={c._id} value={c.fullClassName || c.name}>{c.fullClassName || c.name}</option>)}
        </select>
        <input placeholder="Subject e.g Mathematics" value={selectedSubject} onChange={(e) => setSelectedSubject(e.target.value)} className="bg-slate-900/50 border-slate-800 rounded-lg px-4 py-2 text-sm text-white" />
        <button onClick={handleSaveAll} disabled={saving || !selectedSubject} className="px-5 py-2 bg-emerald-500 hover:bg-emerald-400 disabled:bg-emerald-500/50 text-slate-950 font-semibold rounded-lg flex items-center gap-2">{saving ? <Loader2 className="w-4 h-4 animate-spin" /> : <Save className="w-4 h-4" />} Save All</button>
      </div>
